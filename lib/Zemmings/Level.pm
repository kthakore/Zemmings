...
# my @polygon = map { [ map $_?$_/10:0, map int, split m/,/, $_ ] } grep m/\d/, split m/ /, $polygon;
        my @polygon = map { [ map int, split m/,/, $_ ] } grep m/\d/,
          split m/ /, $polygon;
...
my $floor =
  world->create_body( x => app->w / 2, y => 50, w => app->w, h => 100 );
...
my $protagonist = world->create_body(
    ...
    collided => {
        postsolve => sub { warn 'post solve collision' }
    }
);

my $animation = SDLx::Sprite::Animated->new(
    image  => 'share/stickzombie.png',
    width  => 32,
    height => 32
);
$animation->start;
